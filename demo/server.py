#!/usr/bin/env python3
"""
StayVista Demo — Combined Server
Serves static files on port 8900 AND handles email API at /api/send-confirmation-email
Replaces the separate `python3 -m http.server 8900` + `email-server.py` setup.
"""
...
SMTP_PORT = 587
SMTP_USER = os.environ.get("GMAIL_USER", "kamatanuj@gmail.com")
SMTP_PASSWORD = os.environ.get("GMAIL_APP_PASSWORD", "yapm wbbh wqak vygb")
...
class CombinedHandler(BaseHTTPRequestHandler):
    """Serves static files + handles /api/send-confirmation-email POST."""
    ...
    def do_GET(self):
        ...
        filepath = os.path.normpath(os.path.join(DEMO_DIR, path.lstrip("/")))
        if not filepath.startswith(DEMO_DIR):
...
if __name__ == "__main__":
    print(f"🏠 StayVista Combined Server starting on port {PORT}...")
    print(f"   Static files from: {DEMO_DIR}")
    print(f"   Email API: POST http://localhost:{PORT}/api/send-confirmation-email")
